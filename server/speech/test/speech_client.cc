...
int main(int argc, char *argv[])
{
    ...
    yu::init_logger(FLAGS_run_mode, FLAGS_log_file, FLAGS_log_level);

    
    //1. 先构造Rpc信道管理对象
    auto sm = std::make_shared<yu::ServiceManager>();
    sm->declared(FLAGS_speech_service);
    auto put_cb = std::bind(&yu::ServiceManager::onServiceOnline, sm.get(), std::placeholders::_1, std::placeholders::_2);
    auto del_cb = std::bind(&yu::ServiceManager::onServiceOffline, sm.get(), std::placeholders::_1, std::placeholders::_2);
    //2. 构造服务发现对象
    yu::Discovery::ptr dclient = std::make_shared<yu::Discovery>(FLAGS_etcd_host, FLAGS_base_service, put_cb, del_cb);
    ...
    yu::SpeechRecognitionReq req;
    req.set_speech_content(file_content);
    req.set_request_id("111111");

    brpc::Controller *cntl = new brpc::Controller();
    yu::SpeechRecognitionRsp *rsp = new yu::SpeechRecognitionRsp();
